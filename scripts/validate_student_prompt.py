...
PROMPTS_TO_VALIDATE = ["S-01_student_evaluation", "S-02_student_direct"]
...
def load_prompt_template(prompt_id: str): 
    with open(PROMPTS_CONFIG_PATH, 'r', encoding='utf-8') as f:
        prompts_config = yaml.safe_load(f)
    prompt_info = prompts_config.get(prompt_id)
    if not prompt_info: raise ValueError(f"Prompt ID '{prompt_id}' no encontrado.")
...
def parse_score(text: str) -> (float, bool):
    """Extrae el score y valida el formato. Devuelve (score, formato_ok)."""
    match = re.search(r"Score:\s*(\d+\.?\d*)", text, re.IGNORECASE)
    if match:
        # El formato es correcto. Extrae y devuelve el número.
...
def main():
    ...
    # Cargar y preparar datos de validación
    ...
    # Cargar el modelo Student (Qwen3) con cuantización
    ...
    for prompt_id in PROMPTS_TO_VALIDATE:
        ...
        for _, row in df_full_val.iterrows():
            ...
            prompt_text = prompt_template.format(text_of_cv=cv_text, text_of_job_posting=offer_text)
            
            # Generar respuesta del modelo
            inputs = tokenizer(prompt_text, return_tensors="pt").to(model.device)
            outputs = model.generate(**inputs, max_new_tokens=20)
            result_text = tokenizer.decode(outputs[0], skip_special_tokens=True)
            # Extraer solo la respuesta generada por el modelo
            model_response = result_text[len(prompt_text):].strip()
